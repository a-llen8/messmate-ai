# MESSMATE — COMPLETE PROJECT PLAN v3.0
### Hostel Mess Management System with Agentic AI — Zero Payment Build
> All drawbacks identified and fixed. Every aspect covered. Zero paid services.
...
| Nutrition   | IFCT local DB + Open Food Facts API fallback (free, no auth, no card)     |
...
### Zero-Payment Stack — All Free, No Card Required

| Component | Free Option | Limit |
|-----------|------------|-------|
| LLM | Gemini 2.0 Flash (aistudio.google.com) | 1,500 req/day — MessMate needs ~35/month |
| Nutrition fallback | Open Food Facts API (no key, no auth) | Unlimited |
| Auth + Email | Supabase free tier | 50,000 MAU, 500MB DB |
| Deployment | College server (Docker Compose) | Free |
| Redis (cloud alt.) | Upstash free tier | 10,000 cmd/day |
| Frontend (cloud alt.) | Vercel free tier | Unlimited static |
| Backend (cloud alt.) | Render.com free tier | 750 hrs/month |
...
### Why Open Food Facts over Edamam
- Edamam requires a credit card to register (even free tier)
- Open Food Facts: no account, no key, no card — call the API directly
- Has reasonable Indian food coverage; IFCT handles the bulk anyway
- For unknown custom dishes not found in Open Food Facts, Gemini estimates nutrition as final fallback
...
| Input    | Today's menu_items → IFCT lookup → Open Food Facts fallback → Gemini estimate |
...
def run_agent5_nutrition(menu_id: int):
    ...
    for item in items:
        if item.is_custom:
            # Try Open Food Facts first (free, no auth)
            nutrition = fetch_open_food_facts(item.dish_name_free)
            if not nutrition:
                # Final fallback: Gemini estimates nutrition from dish name
                nutrition = estimate_nutrition_gemini(item.dish_name_free)
        else:
            # IFCT local DB lookup with fuzzy match
            nutrition = lookup_ifct(item.dish_id)
            if not nutrition:
                nutrition = fetch_open_food_facts(item.dish.name)
                if not nutrition:
                    nutrition = estimate_nutrition_gemini(item.dish.name)
...
def lookup_ifct(dish_id: int = None, dish_name: str = None):
    ...
    # Below threshold → Open Food Facts fallback
    return None

def fetch_open_food_facts(dish_name: str):
    """Open Food Facts API — completely free, no key, no account, no card."""
    import requests
    resp = requests.get(
    ...
    if resp.status_code == 200:
        ...
        if data.get("products"):
            n = data["products"][0].get("nutriments", {})
            return {
                "calories": n.get("energy-kcal_100g", 0),
                "protein":  n.get("proteins_100g", 0),
                "fat":      n.get("fat_100g", 0),
                "carbs":    n.get("carbohydrates_100g", 0),
                "fibre":    n.get("fiber_100g", 0),
            }
    return None

def estimate_nutrition_gemini(dish_name: str):
    """
    Final fallback when IFCT and Open Food Facts both miss.
    Uses already-free Gemini quota (negligible calls — only fires for
    truly unknown custom dishes not in any database).
    """
    prompt = f"""Estimate the approximate nutritional values per 100g for the Indian dish: "{dish_name}".
Return ONLY this JSON, no other text:
{{
  "calories": <number>,
  "protein": <number>,
  ...
  "fibre": <number>
    ...
    try:
        ...
        return None   # give up gracefully; score will be partial
```
...
### Nutrition Lookup Fallback Chain (Zero-Cost)
```
1. IFCT local DB (fuzzy match, rapidfuzz) — covers 528 Indian foods
2. Open Food Facts API (free, no auth, no card)
3. Gemini estimate (free quota, used only as last resort)
4. Return None → score computed from available dishes only
```
No external paid API at any point in the chain.
...
### Razorpay (Not in scope — payment is offline only)
Online payment gateway is excluded from all phases of this project to keep zero-payment infrastructure. If the system is later commercialized, Razorpay can be added at that point.
...
Tasks:
  ...
  ✓ Agent 5 implementation + test
      ...
      Test Open Food Facts fallback + Gemini nutrition estimate
...
Tasks:
  ...
  ✓ Deploy Docker Compose on college server (or Render+Upstash+Vercel free tier)
...
### Option A — College Server (Recommended, Zero Cost)
Run Docker Compose directly on a college-provided server or lab machine.
No cloud account needed. Most reliable zero-cost path.

```
Requirements: Any Linux machine with Docker installed
              (2GB RAM minimum, 20GB disk)
Access: SSH from dev machine → docker-compose up -d
Domain: Use college's internal network IP, or request a subdomain
HTTPS: Self-signed cert for internal use, or Let's Encrypt if public domain available
```
...
### Option B — Free Cloud (Zero Cost, No Card)

If no college server is available, use this free-tier cloud stack:

| Service | What it runs | Free limit |
|---------|-------------|------------|
| Supabase | PostgreSQL + Auth + Email | 500MB DB, 50K MAU |
| Upstash | Redis (Celery broker) | 10,000 cmd/day |
| Render.com | FastAPI (web service) + Celery worker | 750 hrs/month |
| Vercel | React frontend | Unlimited |

**Render caveat:** Free web services spin down after 15 min of inactivity (cold start ~30s). Acceptable for a pilot/demo. If this is an issue, use the college server instead.

**Upstash setup (replaces Redis container):**
```python
# In .env
REDIS_URL=rediss://:password@your-upstash-endpoint:6380/0
# Celery picks this up automatically from REDIS_URL
```

### Minimum Hardware (College Server Path)
```
Any Linux machine: 2 vCPU, 2GB RAM, 20GB SSD
...
REDIS_URL=redis://redis:6379/0          # or Upstash URL for cloud deployment
SUPABASE_URL=https://xyz.supabase.co
SUPABASE_KEY=your_service_key
GEMINI_API_KEY=your_gemini_key          # aistudio.google.com — free, no card
...
|-----------|-------|-------|
| Online payments (Razorpay/etc.) | Only if commercialized | Not planned — system is offline payment only |
| WhatsApp Business alerts | Optional future | Has per-message cost; skip unless funded |
...
- [ ] aistudio.google.com → get free Gemini API key (no card needed)
- [ ] supabase.com → create free project + configure email SMTP
...
- [ ] Open Food Facts fallback tested with 5 custom dishes
- [ ] All .env vars confirmed — confirm no paid API keys present
- [ ] Deployment target confirmed: college server OR Render+Upstash+Vercel

**Zero-payment verification:** Before launch, confirm these are NOT in .env:
`EDAMAM_APP_ID`, `EDAMAM_APP_KEY`, `RAZORPAY_KEY`, any paid service key.

---

*MessMate v3.0 — Zero payment. All drawbacks resolved. Build-ready.*
